<template>
  <div class="top_" v-if="!props.title">
    ...
    <div class="right" v-if="isLogin == true">
      <div>
        <a-badge :count="num" @click="router.push('./news')">
...
<script setup lang="ts">
  ...
  import { api_logout, api_problemFeedback, api_newsList } from '@/requset/api'
  ...
  api_newsList({ type: 1 }).then((res: any) => {
    if (res.success) {
      noticeList.value.push(res.data)
    }
  })
  api_newsList({ type: 2 }).then((res: any) => {
    if (res.success) {
      noticeList.value.push(res.data)
    }
  })
  let num = ref<number>(0)
  noticeList.value.forEach((x: any) => {
    if (x.is_read != 1) {
      num.value++
    }
  })
